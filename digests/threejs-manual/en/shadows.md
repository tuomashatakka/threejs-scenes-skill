<!-- ingested from https://threejs.org/manual/en/shadows.html (direct markdown, no model) -->
...
consider starting there. The
 previous article was about cameras which is
important to have read before you read this article as well as
the article before that one about lights . 
...
We'll go over both of these issues in another article .
...
of shadow is Animal Crossing Pocket Camp 
...
that is looking at its target. Just like the camera's we previously covered 
...
We went over how an OrthographicCamera works in the previous article about cameras . 
...
```
 ...
 We'll also use the MinMaxGUIHelper we created in the camera article 
...
allowed which is available on the renderer as renderer.capabilities.maxTextureSize . 
