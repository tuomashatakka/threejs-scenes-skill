<!-- ingested from https://threejs.org/manual/en/cameras.html (direct markdown, no model) -->
...
multiplied by the aspect. 
 ...
 Let's use the scene from the previous article that has a ground
...
the plane so its center appears to be at its top left corner. 
 If you're running locally you'll also need to have setup .
You might also want to read about using textures . 
...
in other articles. For now let's move on to shadows .
