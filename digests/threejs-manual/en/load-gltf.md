<!-- ingested from https://threejs.org/manual/en/load-gltf.html (direct markdown, no model) -->

Loading a .GLTF File 

 In a previous lesson we loaded an .OBJ file . If
...
into might provide some good info. 
 Searching the net I found this low-poly city 
by antonmoek which seemed like if we're lucky
might make a good example. 

 Starting with an example from the .OBJ article I removed the code
...
```
 ...
 Running that I got this listing 
...
the lines made by connecting those 250 points. 
 Running the example I didn't see
...
example in the article on shadows and pasted it
