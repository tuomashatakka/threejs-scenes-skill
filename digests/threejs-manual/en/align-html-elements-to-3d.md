...
each with pluses and minuses. 
 ...
 Use 3D text 
 If you look at the primitives article you'll see TextGeometry which
...
or labelling lots of things. 
 ...
 The article on using a Canvas as a texture shows using
a canvas as a texture. You can draw text into a canvas and display it as a billboard .
...
with an example from the article on responsive pages 
...
the picking code from the article on picking .
...
article but that is also not free. Which solution you
...
the page we can tell the browser to create a new stacking context 
...
objects though we are not drawing lots
...
```
 ...
 In the article on optimizing lots of objects 
