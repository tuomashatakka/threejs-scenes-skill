<!-- ingested from https://threejs.org/manual/en/shadertoy.html (direct markdown, no model) -->

Three.js and Shadertoy 

 Shadertoy is a famous website hosting amazing shader
...
similar to say dwitter (write code in 140 characters) or
 js13kGames (make a game in 13k or less). 
...
and maybe follow that up with this series . 
 It should be noted that, at least as of January 2019,
 shadertoy.com only concerns itself with fragment
...
shadertoy made up. 
 The Shadertoy docs define several more . For
...
haven't read it yet we did this in the article on backgrounds 
...
```
 ...
 As explained in the backgrounds article an
...
```
 ...
 is not documented on shadertoy.com . It's
not used above so just setting it to 1 for now. ¯\_(ツ)_/¯ 

 click here to open in a separate window 

 This matches what we see on Shadertoy for a new shader ,
...
```
 ...
 passing one into a normal material but we need to set
...
```
 ...
 Shadertoy.com , namely drawing to cover the canvas.
...
cubes from the article on responsiveness . The result: 
