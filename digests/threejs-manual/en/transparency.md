<!-- ingested from https://threejs.org/manual/en/transparency.html (direct markdown, no model) -->
...
scene with 8 cubes placed in a 2x2x2 grid. 
 ...
 the article on rendering on demand 
...
```
 ...
 transparent flag and to set an
 opacity level with 1 being completely opaque
...
missing their backs. 
 ...
 We learned about the side material property in
...
out others. Unfortunately for individual triangles shorting would be extremely slow. 
 ...
 the same order they are built in the geometry 
...
We'll add a texture to each plane. 
...
This is because this sample is rendering on demand 
...
```
 ...
 Blender I'd probably do this manually by adjusting
...
before By setting the texture.repeat 
and texture.offset we can scale and move the texture to get
...
use these textures and set an alphaTest . 
...
in the article on three.js's scenegraph . 
