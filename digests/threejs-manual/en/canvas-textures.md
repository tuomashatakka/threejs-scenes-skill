<!-- ingested from https://threejs.org/manual/en/canvas-textures.html (direct markdown, no model) -->

Canvas Textures 

 This article continues from the article on textures .
If you haven't read that yet you should probably start there. 
 In the previous article on textures we mostly used
...
draw with the 2D canvas API on a canvas there's a good tutorial on MDN . 
...
a cube from the previous article .
...
better off using a RenderTarget which is covered in this article . 
...
stuff. Then let's set the background to white and add two lights . 
...
We'll cover how to do that in an article on billboards . 
 For labels in particular, another solution is to use HTML .
The labels in this article are inside the 3D world which is good if you want them
to be hidden by other objects where as HTML labels are always on top.
