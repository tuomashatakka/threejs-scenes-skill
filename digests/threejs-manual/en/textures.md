<!-- ingested from https://threejs.org/manual/en/textures.html (direct markdown, no model) -->

Textures 

 This article is one in a series of articles about three.js.
The first article was about three.js fundamentals .
The previous article was about setting up for this article.
...
put this image on cube. 
 ...
 load method with the URL of an
...
We create a TextureLoader and then call its load method.
...
and pass it to the TextureLoader then set its onLoad 
...
```
 ...
 The LoadingManager also has an onProgress property
...
permission headers then you can't use the images from that server. 
 For example imgur , flickr , and
 github all send headers allowing you to use images
...
you set texture.magFilter property to either THREE.NearestFilter or
...
you set the texture.minFilter property to one of 6 values. 
...
texture repeats there are 2 properties, wrapS for horizontal wrapping
and wrapT for vertical wrapping. 
...
```
 ...
 wrapT on the texture you must also set texture.needsUpdate 
