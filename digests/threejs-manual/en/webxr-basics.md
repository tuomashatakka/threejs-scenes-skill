<!-- ingested from https://threejs.org/manual/en/webxr-basics.html (direct markdown, no model) -->
...
has a different field of view and display aspect. 
 Let's take an example from the article on making a responsive webpage 
...
the WebXR API. The server mentioned in the article on setting up 
called Servez has an option to use https.
...
room for reference so let's add a simple grid cubemap like we covered in
 the article on backgrounds . We'll just use the same grid
...
For iOS you might be able to use this WebXR App 
...
cheap recommendations made from cardboard 
...
need to turn on WebXR support in about:flags . I also know Google
Daydream will also work and similarly you need to enable WebXR support in
 about:flags . Oculus Rift, Vive, and Vive Pro will work via
