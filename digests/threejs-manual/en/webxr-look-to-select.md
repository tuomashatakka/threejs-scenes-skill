<!-- ingested from https://threejs.org/manual/en/webxr-look-to-select.html (direct markdown, no model) -->

VR - Look to Select 

 NOTE: The examples on this page require a VR capable
device. Without one they won't work. See previous article 
as to why 
...
and to do it we'll add the PickHelper we made in
 the article on picking . Here it is. 
...
that to a value that goes from -0.5 to 0.5 
 Textures are mapped to geometry using normalized texture coordinates
...
covered in the article on backgrounds .
