<!-- ingested from https://threejs.org/manual/en/cleanup.html (direct markdown, no model) -->
...
```
 ...
 Let's use this class with the first example from the article on textures .
...
```
 ...
 And with that let's take an example from the article on loading gltf files 
