<!-- ingested from https://threejs.org/manual/en/optimize-lots-of-objects.html (direct markdown, no model) -->

Optimize Lots of Objects 

 This article is part of a series of articles about three.js. The first article
is three.js fundamentals . If you haven't read that
...
demand We'll make one box per data in the
file. 
...
because we're rendering on demand instead of
...
objects like we covered in scene graphs but the more
...
were to place the box. 
 ...
 Above the green bar represents lonHelper and
is used to rotate toward longitude on the equator. The 
blue bar represents latHelper which is used to rotate to a
latitude above or below the equator. The red
sphere represents the offset that that positionHelper provides. 
...
once since we're rendering on demand . 
...
devtools and turning on the browser's frame
rate meter. 
