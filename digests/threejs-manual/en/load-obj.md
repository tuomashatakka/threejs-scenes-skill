<!-- ingested from https://threejs.org/manual/en/load-obj.html (direct markdown, no model) -->
...
3D format so let's try loading one. 
 Searching the net I found this CC-BY-NC 3.0 windmill 3D model by ahedov . 

 I downloaded the .blend file from that site, loaded it into Blender 
...
to really go through some lessons. 
 ...
 the lights article and I combined it with
...
we went over in the article on materials .
...
the surface. 
 ...
 Looking at the source for the MTLLoader 
...
we can see the back of the blades. 
 ...
 Searching the net I found this CC-BY-NC windmill 3D model made by Roger Gerzner / GERIZ.3D Art . 
...
altitude so we'll just have to pick something. 
 As we went over in the article on cameras the camera defines a frustum.
...
inside the frustum. 
 Using basic right triangle trigonometry and SOHCAHTOA ,
...
find on the net then maybe, just maybe, you might want to load TGA files. ( * ) 
...
the textures. They are currently a total of 10meg!!! 
 Also remember
like we mentioned in the article on textures that
