<!-- ingested from https://threejs.org/manual/en/indexed-textures.html (direct markdown, no model) -->

Indexed Textures for Picking and Color 

 This article is a continuation of an article about aligning html elements to 3d .
...
We could use a picking solution like we covered before.
...
Or we could use something like draco compression 
...
we covered at the end of the article on picking . In
...
and is therefore licensed as CC-BY-SA . 
...
even lower the resolution but 217k seems good enough for now. 
 ...
 Grabbing code from the gpu picking example we need
a scene for picking. 
...
That way we can use lighting and other features if we want. 
 Like we covered in the article on animating lots of objects 
...
base material color. It sets this in the <color_fragment> snippet 
...
this article. There are a few links to some info in
 the article on post processing .
