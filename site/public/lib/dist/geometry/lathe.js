...
export function createLatheMesh(profile, options = {}) {
    const { segments = 32, phiStart = 0, phiLength = Math.PI * 2, material = new THREE.MeshStandardMaterial({ color: '#c0c8d0', roughness: 0.5, metalness: 0.2 }), } = options;
    const pts = profile.map(p => p instanceof THREE.Vector2 ? p : new THREE.Vector2(p[0], p[1]));
