...
`;
export function createShaderQuad({ fragmentShader, uniforms = {}, pointerElement }) {
    ...
    if (pointerElement) {
        const onMove = (e) => {
            ...
            material.uniforms.uPointer.value.set((e.clientX - rect.left) / rect.width * 2 - 1, -((e.clientY - rect.top) / rect.height * 2 - 1));
        };
        pointerElement.addEventListener('pointermove', onMove);
        detach = () => pointerElement.removeEventListener('pointermove', onMove);
    }
    const size = new THREE.Vector2();
    function update({ elapsed }, renderer) {
...
//# sourceMappingURL=shader-quad.js.map