 ...
 */
export function sampleCurve (curve: ScalarCurve, t: number): number {
  const first = curve[0]
...
export function bakeCurve (curve: ScalarCurve, resolution = 64): Float32Array {
...
export function bakeCurveTexture (
  ...
  for (let i = 0; i < resolution; i++) {
    ...
    for (let s = 0; s < color.length; s++) {
      const stop = color[s] as ColorCurve[number]
      if (stop[0] >= t) {
        stopIndex = s
        break
      }
    ...
    const b         = color[stopIndex] as ColorCurve[number]
    const a         = color[Math.max(0, stopIndex - 1)] as ColorCurve[number]
    const span      = b[0] - a[0]
...
function toColor (value: string | readonly [number, number, number]): readonly [number, number, number] {
