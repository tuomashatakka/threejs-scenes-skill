...
import type { Emitter, EmitterOptions, SpawnSample } from './emitter.js'
...
export function createGpuEmitter (renderer: THREE.WebGLRenderer, options: GpuEmitterOptions): Emitter {
  ...
  const sample: SpawnSample = { px: 0, py: 0, pz: 0, dx: 0, dy: 1, dz: 0 }
  const posData             = positionTex.image.data as Float32Array
  const velData             = velocityTex.image.data as Float32Array
  for (let i = 0; i < count; i++) {
    ...
    velData[ix + 0]      = spawnVelData[ix + 0] as number
    velData[ix + 1]      = spawnVelData[ix + 1] as number
    velData[ix + 2]      = spawnVelData[ix + 2] as number
    velData[ix + 3]      = 0
  }
  ...
  function tick ({ delta }: FrameContext): void {
    (positionVar.material.uniforms.uDelta as THREE.IUniform).value = delta;
    (velocityVar.material.uniforms.uDelta as THREE.IUniform).value = delta;
    (material.uniforms.uPixelScale as THREE.IUniform).value        = renderer.domElement.height
    compute.compute();
    (material.uniforms.texturePosition as THREE.IUniform).value = compute.getCurrentRenderTarget(positionVar).texture
  }

  return {
    object: points,
    tick,
    burst () { /* steady-state field: bursts are a CPU-emitter feature */ },
    setRate () { /* steady-state field: rate is fixed at capacity/lifetime */ },
    dispose () {
      compute.dispose()
      geometry.dispose()
      material.dispose()
