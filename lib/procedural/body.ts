...
export interface ProceduralBodySpec {
  ...
  palette?: { low: THREE.ColorRepresentation; mid: THREE.ColorRepresentation; high: THREE.ColorRepresentation }
  water?:   { level: number; color: THREE.ColorRepresentation } | null
  clouds?:  { coverage?: number; color?: THREE.ColorRepresentation } | null
...
export function createProceduralBody ({
  ...
  octaves = 5,
  ridged = false,
  palette = { low: '#2d6a4f', mid: '#8a7f5c', high: '#e8e6e0' },
  water = { level: 0, color: '#1d4e89' },
  clouds = { coverage: 0.5, color: '#ffffff' },
  rings = null,
}: ProceduralBodySpec = {}): ProceduralBody {
  ...
  for (let i = 0; i < position.count; i++) {
    ...
    if (type === 'gas') {
      ...
      position.setXYZ(
        i,
        ...vertex.multiplyScalar(radius * (1 + Math.max(0, n) * displacement)).toArray(),
      )
    }
  ...
  if (clouds) {
    ...
    const cloudPos   = cloudGeo.getAttribute('position') as THREE.BufferAttribute
