...
export function createNoiseTexture({ size = 256, frequency = 4, octaves = 4, seed = 1, channels = 'rgba', } = {}) {
    ...
    for (let y = 0; y < size; y++)
        for (let x = 0; x < size; x++) {
            ...
            for (let o = 0; o < octaves; o++) {
                ...
                total += amp * noise(Math.cos(angleU) * freq, Math.sin(angleU) * freq, Math.cos(angleV) * freq);
                total += amp * 0.5 * noise(Math.cos(angleU) * freq * 1.7, Math.sin(angleV) * freq * 1.7, Math.cos(angleU + angleV) * freq * 1.3);
                norm += amp * 1.5;
                amp *= 0.5;
                freq *= 2;
            }
...
//# sourceMappingURL=noise-texture.js.map