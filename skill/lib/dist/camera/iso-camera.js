...
export function createIsoCamera(aspect, { viewSize = 20, flavor = 'dimetric', near = 0.1, far = 500, } = {}) {
    const h = viewSize / 2;
    const w = h * aspect;
    ...
    const radius = 100;
    camera.position.set(Math.cos(THREE.MathUtils.degToRad(45)) * radius * Math.cos(tilt), Math.sin(tilt) * radius, Math.sin(THREE.MathUtils.degToRad(45)) * radius * Math.cos(tilt));
    camera.lookAt(0, 0, 0);
    const data = { viewSize, flavor };
    camera.userData.viewSize = data.viewSize;
    camera.userData.flavor = data.flavor;
    return camera;
}
export function resizeIsoCamera(camera, aspect) {
    const viewSize = camera.userData.viewSize;
    const h = viewSize / 2;
    const w = h * aspect;
    camera.left = -w;
    camera.right = w;
...
//# sourceMappingURL=iso-camera.js.map