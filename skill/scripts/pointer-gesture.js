// scripts/pointer-gesture.js
// Unified pointer gesture layer. Drag, pinch, wheel, tap — works identically
// on mouse, touch, pen, stylus. Always use this instead of mouse*/touch* events.

export function attachPointerGesture (el, callbacks, {
  tapThresholdMs = 250,
  tapMovePx = 8,
} = {}) {
  ...
  const onMove = e => {
    ...
    if (pointers.size === 1 && callbacks.onDrag)
      callbacks.onDrag(p.x - p.lastX, p.y - p.lastY, e); else if (pointers.size === 2 && callbacks.onPinch) {
      const [ a, b ] = [ ...pointers.values() ]
