...
export const scenePlanSchema = z.object({
  ...
  features: z.array(z.object({
    ...
    countEstimate: z.number().int().positive().optional(),
  })),
...
export const voxelLevelSchema = z.object({
  chunkSize: z.number().int().min(8).max(64),
  ...
  layers: z.array(z.object({
    ...
    voxelId: z.number().int().min(1).max(65535),
  })),
})
...
export const proceduralTextureSchema = z.object({
  size: z.number().int().refine(n => (n & (n - 1)) === 0, 'must be power of two'),
